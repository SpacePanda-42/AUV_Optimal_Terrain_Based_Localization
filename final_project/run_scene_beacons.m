...
dt = 1;
rangeAccuracy = 0.5; % standard deviation for beacon measurements
AUV_altitude = 80;
...
for idx = 1:trajectoryLength
    ...
    if idx>0
        if isupdatedLidar
            p = trajectory(idx,1:2);
            meas1 = norm(p-m1);
            meas2 = norm(p-m2);
            meas3 = norm(p-m3);
            meas4 = norm(p-m4);
            measurement = [meas1, meas2, meas3, meas4];
            ...
            for m = 1:N_particles
                ...
                p = [particle(1), particle(2)];
                pred_meas1 = norm(p-m1);
                pred_meas2 = norm(p-m2);
                pred_meas3 = norm(p-m3);
                pred_meas4 = norm(p-m4);

                predicted_measurement = [pred_meas1, pred_meas2, pred_meas3, pred_meas4];
...
save beacon_mu_est_hist.mat mu_hist
save beacon_true_path.mat trajectory
